...
function tests.test_intermediate_nr_consistency()
    ...
    local p2 = point.new(vector.new(30, 25, 20))
    local pth = path.new(p1, p2)
    
    assert(pth.intermediate_nr == 0, "Initial intermediate_nr should be 0")
    
    -- Insert some points
    pth:insert(point.new(vector.new(8, 10, 12)))
    pth:insert(point.new(vector.new(15, 15, 15)))
    pth:insert(point.new(vector.new(22, 20, 18)))
...
function tests.test_split_preserves_count()
    ...
    pth:insert(point.new(vector.new(35, 18, 26)))
    
    local split_point = pth:get_point(2)
...
function tests.test_extend_shorten_count()
    ...
    pth:extend(point.new(vector.new(45, 50, 55)))
...
function tests.test_unsubdivide_count()
    ...
    -- Create a straight line with multiple collinear points
    pth:insert(point.new(vector.new(4, 2, 6)))
    pth:insert(point.new(vector.new(8, 4, 12)))
    pth:insert(point.new(vector.new(12, 6, 18)))
    pth:insert(point.new(vector.new(16, 8, 24)))
...
function tests.test_path_unsubdivide()
    ...
    assert(pth.intermediate_nr == 3, "Should start with 3 intermediate points")
...
function tests.test_path_make_wave()
    local p1 = point.new(vector.new(0, 0, 0))
    local p2 = point.new(vector.new(100, 0, 50))
    local pth = path.new(p1, p2)
    ...
    -- Check that some points are offset from the straight line (perpendicular displacement)
    local has_offset = false
    for _, p in ipairs(pth:all_points()) do
        -- The wave should create y-offsets (perpendicular to xz plane direction)
        if math.abs(p.pos.y) > 0.01 then
            has_offset = true
            break
        end
    end
...
function tests.test_path_make_slanted()
    ...
    assert(pth3.intermediate_nr == 0, "Z-aligned path should have no intermediate points")
end
