...
-- ============================================================
-- ROAD ORIGIN GENERATION (with varying Y levels)
-- ============================================================

-- Get all grid-aligned points along a citychunk border with varying y levels
local function get_grid_points_on_border(citychunk_origin, border_edge)
    ...
    -- Vary y level based on position for testing 3D roads
    ...
    if border_edge == "x_min" or border_edge == "x_max" then
        ...
        while z <= max_z do
            if z > min_z and z < max_z then
                -- Vary y level for 3D roads
                local y = ground_level + y_offset
                table.insert(points, vector.new(x, y, z))
                y_offset = (y_offset + 5) % 20  -- Vary between 0 and 15
            end
            z = z + grid_spacing
        end
    else
...
local function get_road_origin_on_border(citychunk_origin, border_edge, salt)
    ...
    if #grid_points == 0 then
        ...
        local min_z = citychunk_origin.z
        local max_z = citychunk_origin.z + citychunk.in_nodes.z - 1
        local y = units.sizes.ground_level
...
function pcmg.citychunk_road_origins_3d(citychunk_origin)
...
-- Create 3D shapes (circles at y=0 relative to cursor)
...
-- ============================================================
-- ROAD BUILDING (with 3D path)
-- ============================================================

-- Builds a simple road path between start and finish points.
-- The path now varies in y level to demonstrate 3D roads.
local function build_road_3d(start, finish)
    ...
    -- Add intermediate points with varying y levels
    local current = guide_path.start
    while current.next do
        local next_pos = current.next.pos
        local current_pos = current.pos
        local distance = vector.distance(current_pos, next_pos)
        
        -- Add intermediate points every 80 nodes to create elevation changes
        if distance > 80 then
            local mid_pos = vector.add(current_pos, vector.divide(vector.subtract(next_pos, current_pos), 2))
            -- Vary y level to create ramps
            mid_pos.y = (current_pos.y + next_pos.y) / 2
            local mid_point = pcmg.point.new(mid_pos)
            guide_path:insert_after(current, mid_point)
        end
...
-- ============================================================
-- MAIN GENERATION (for 3D)
-- ============================================================

local function road_generator_3d(megacanv, pathpaver_cache)
    megacanv:set_metastore(road_metastore)
    
    set_citychunk_seed(megacanv.origin, 0)
    
    local road_origins = pcmg.citychunk_road_origins_3d(megacanv.origin)
    ...
    for _, points in ipairs(connected_points) do
        ...
        local pth = build_road_3d(start, finish)
...
function pcmg.generate_roads_3d(megacanv, pathpaver_cache)
    local t1 = core.get_us_time()
    megacanv:generate(road_generator_3d, 1, pathpaver_cache)
end
