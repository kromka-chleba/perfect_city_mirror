...
-- Write roads from a 3D canvas to the voxel manipulator
function pcmg.write_roads_3d(mapgen_args, canv3d)
    ...
    local array_min, array_max = canv3d:mapchunk_indices(pos_min, pos_max)
    
    -- Iterate through the 3D array
    for x = array_min.x, array_max.x do
        for y = array_min.y, array_max.y do
            for z = array_min.z, array_max.z do
                local cell_id = canv3d.array[x] and canv3d.array[x][y] and canv3d.array[x][y][z]
                if cell_id and cell_id ~= blank_id then
                    local abs_pos = pos_min + vector.new(x, y, z) - array_min
                    
                    -- Check if position is within the emerged area
                    if abs_pos.x >= emin.x and abs_pos.x <= emax.x and
                       abs_pos.y >= emin.y and abs_pos.y <= emax.y and
                       abs_pos.z >= emin.z and abs_pos.z <= emax.z then
    ...
    -- Write data
    vm:set_data(data)
    --core.log("error", string.format("3D chunk writing time: %g ms", (core.get_us_time() - t1) / 1000))
end
